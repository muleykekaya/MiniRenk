Feux.Validation = {
  ...
  Actions: {
    init: function () {
      ...
      jQuery.validator.setDefaults({
        errorPlacement: function (error, element) {
          ...
          if (listContainer) {
            ...
            listContainer
              .find('input.error')
              .removeClass('error')
              .addClass('no-validation-styling');
          }
        },
        highlight: function (element, errorClass, validClass) {
          $(element)
            ...
            .parent()
            .addClass('parent' + errorClass)
            .removeClass('parent' + validClass);
        },
        unhighlight: function (element, errorClass, validClass) {
          $(element)
            ...
            .addClass(validClass)
            .parent()
            .removeClass('parent' + errorClass)
            .addClass('parent' + validClass);

          $(element)
            .parents('.list-validation')
        ...
        invalidHandler: function (form, validator) {
          ...
          if (Feux.Modal.Current.state === 'on') {
            $('html, body').animate(
              {
                scrollTop: parseInt(
                  $(validator.errorList[0].element).offset().top - 200
                ),
              },
              timerVal
            );
      ...
      $.validator.addMethod(
        ...
        function (val, elem) {
          ...
          if (dependentElemProps.tag === 'select') {
            ...
            dependentElemProps.type === 'checkbox' ||
            dependentElemProps.type === 'radio'
          ) {
          ...
          if (checkElIsValid) {
            if (
              senderElemProps.type === 'text' ||
              senderElemProps.tag === 'textarea' ||
              senderElemProps.tag === 'multiSelect' ||
              senderElemProps.tag === 'select'
            ) {
              ...
              var selectedEl = document.querySelector(
                'input[name="' + senderElemProps.name + '"]:checked'
              );
              ret = selectedEl !== null;
            }
          }
      ...
      $.validator.addMethod('turkishDate', function (currVal, element) {
        ...
        if (
          parseInt(dtYear) > parseInt(dateRange[1]) ||
          parseInt(dtYear) < parseInt(dateRange[0])
        ) {
          return false;
        }
      ...
      jQuery.extend(jQuery.validator.messages, {
        ...
        max: jQuery.validator.format(
          '{0} rakamına eşit ya da daha küçük bir değer giriniz.'
        ),
        min: jQuery.validator.format(
          '{0} rakamına eşit ya da daha büyük bir değer giriniz.'
        ),
